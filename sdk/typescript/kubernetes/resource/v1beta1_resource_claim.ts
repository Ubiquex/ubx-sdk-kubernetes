// Code generated by `ubx sdk gen` from kubernetes_resource_v1beta1_resource_claim. DO NOT EDIT.
import type { Computed, FieldMap, ResourceBinding } from "@ubx/sdk";

export interface V1beta1ResourceClaim_Metadata_ManagedFields {
  apiVersion?: string | Computed<string>;
  fieldsType?: string | Computed<string>;
  fieldsV1?: unknown | Computed<unknown>;
  manager?: string | Computed<string>;
  operation?: string | Computed<string>;
  subresource?: string | Computed<string>;
  time?: string | Computed<string>;
}

export interface V1beta1ResourceClaim_Metadata_OwnerReferences {
  apiVersion?: string | Computed<string>;
  blockOwnerDeletion?: boolean | Computed<boolean>;
  controller?: boolean | Computed<boolean>;
  kind?: string | Computed<string>;
  name?: string | Computed<string>;
  uid?: string | Computed<string>;
}

export interface V1beta1ResourceClaim_Metadata {
  /** Annotations is an unstructured key value map stored with a resource that may be set by external tools to store and retrieve arbitrary metadata. They are not queryable and should be preserved when modifying objects. More info: https://kubernetes.io/docs/concepts/overview/working-with-objects/annotations */
  annotations?: Record<string, string> | Computed<Record<string, string>>;
  /** Time is a wrapper around time.Time which supports correct marshaling to YAML and JSON. Wrappers are provided for many of the factory methods that the time package offers. */
  creationTimestamp?: string | Computed<string>;
  /** Number of seconds allowed for this object to gracefully terminate before it will be removed from the system. Only set when deletionTimestamp is also set. May only be shortened. Read-only. */
  deletionGracePeriodSeconds?: number | Computed<number>;
  /** Time is a wrapper around time.Time which supports correct marshaling to YAML and JSON. Wrappers are provided for many of the factory methods that the time package offers. */
  deletionTimestamp?: string | Computed<string>;
  /** Must be empty before the object is deleted from the registry. Each entry is an identifier for the responsible component that will remove the entry from the list. If the deletionTimestamp of the object is non-nil, entries in this list can only be removed. Finalizers may be processed and removed in any order. Order is NOT enforced because it introduces significant risk of stuck finalizers. finalizers is a shared field, any actor with permission can reorder it. If the finalizer list is processed in order, then this can lead to a situation in which the component responsible for the first finalizer in the list is waiting for a signal (field value, external system, or other) produced by a component responsible for a finalizer later in the list, resulting in a deadlock. Without enforced ordering finalizers are free to order amongst themselves and are not vulnerable to ordering changes in the list. */
  finalizers?: string[] | Computed<string[]>;
  /** GenerateName is an optional prefix, used by the server, to generate a unique name ONLY IF the Name field has not been provided. If this field is used, the name returned to the client will be different than the name passed. This value will also be combined with a unique suffix. The provided value has the same validation rules as the Name field, and may be truncated by the length of the suffix required to make the value unique on the server. If this field is specified and the generated name exists, the server will return a 409. Applied only if Name is not specified. More info: https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#idempotency */
  generateName?: string | Computed<string>;
  /** A sequence number representing a specific generation of the desired state. Populated by the system. Read-only. */
  generation?: number | Computed<number>;
  /** Map of string keys and values that can be used to organize and categorize (scope and select) objects. May match selectors of replication controllers and services. More info: https://kubernetes.io/docs/concepts/overview/working-with-objects/labels */
  labels?: Record<string, string> | Computed<Record<string, string>>;
  /** ManagedFields maps workflow-id and version to the set of fields that are managed by that workflow. This is mostly for internal housekeeping, and users typically shouldn't need to set or understand this field. A workflow can be the user's name, a controller's name, or the name of a specific apply path like "ci-cd". The set of fields is always in the version that the workflow used when modifying the object. */
  managedFields?: V1beta1ResourceClaim_Metadata_ManagedFields[] | Computed<V1beta1ResourceClaim_Metadata_ManagedFields[]>;
  /** Name must be unique within a namespace. Is required when creating resources, although some resources may allow a client to request the generation of an appropriate name automatically. Name is primarily intended for creation idempotence and configuration definition. Cannot be updated. More info: https://kubernetes.io/docs/concepts/overview/working-with-objects/names#names */
  name?: string | Computed<string>;
  /** Namespace defines the space within which each name must be unique. An empty namespace is equivalent to the "default" namespace, but "default" is the canonical representation. Not all objects are required to be scoped to a namespace - the value of this field for those objects will be empty. Must be a DNS_LABEL. Cannot be updated. More info: https://kubernetes.io/docs/concepts/overview/working-with-objects/namespaces */
  namespace?: string | Computed<string>;
  /** List of objects depended by this object. If ALL objects in the list have been deleted, this object will be garbage collected. If this object is managed by a controller, then an entry in this list will point to this controller, with the controller field set to true. There cannot be more than one managing controller. */
  ownerReferences?: V1beta1ResourceClaim_Metadata_OwnerReferences[] | Computed<V1beta1ResourceClaim_Metadata_OwnerReferences[]>;
  /** An opaque value that represents the internal version of this object that can be used by clients to determine when objects have changed. May be used for optimistic concurrency, change detection, and the watch operation on a resource or set of resources. Clients must treat these values as opaque and passed unmodified back to the server. They may only be valid for a particular resource or set of resources. Populated by the system. Read-only. Value must be treated as opaque by clients and . More info: https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#concurrency-control-and-consistency */
  resourceVersion?: string | Computed<string>;
  /** Deprecated: selfLink is a legacy read-only field that is no longer populated by the system. */
  selfLink?: string | Computed<string>;
  /** UID is the unique in time and space value for this object. It is typically generated by the server on successful creation of a resource and is not allowed to change on PUT operations. Populated by the system. Read-only. More info: https://kubernetes.io/docs/concepts/overview/working-with-objects/names#uids */
  uid?: string | Computed<string>;
}

export interface V1beta1ResourceClaim_Spec_Devices_Config_Opaque {
  driver?: string | Computed<string>;
  parameters?: unknown | Computed<unknown>;
}

export interface V1beta1ResourceClaim_Spec_Devices_Config {
  opaque?: V1beta1ResourceClaim_Spec_Devices_Config_Opaque | Computed<V1beta1ResourceClaim_Spec_Devices_Config_Opaque>;
  requests?: string[] | Computed<string[]>;
}

export interface V1beta1ResourceClaim_Spec_Devices_Constraints {
  distinctAttribute?: string | Computed<string>;
  matchAttribute?: string | Computed<string>;
  requests?: string[] | Computed<string[]>;
}

export interface V1beta1ResourceClaim_Spec_Devices_Requests_Capacity {
  requests?: Record<string, string> | Computed<Record<string, string>>;
}

export interface V1beta1ResourceClaim_Spec_Devices_Requests_DerivedAttributes {
  expression?: string | Computed<string>;
  name?: string | Computed<string>;
}

export interface V1beta1ResourceClaim_Spec_Devices_Requests_FirstAvailable_Selectors_Cel {
  expression?: string | Computed<string>;
}

export interface V1beta1ResourceClaim_Spec_Devices_Requests_FirstAvailable_Selectors {
  cel?: V1beta1ResourceClaim_Spec_Devices_Requests_FirstAvailable_Selectors_Cel | Computed<V1beta1ResourceClaim_Spec_Devices_Requests_FirstAvailable_Selectors_Cel>;
}

export interface V1beta1ResourceClaim_Spec_Devices_Requests_FirstAvailable_Tolerations {
  effect?: string | Computed<string>;
  key?: string | Computed<string>;
  operator?: string | Computed<string>;
  tolerationSeconds?: number | Computed<number>;
  value?: string | Computed<string>;
}

export interface V1beta1ResourceClaim_Spec_Devices_Requests_FirstAvailable {
  allocationMode?: string | Computed<string>;
  capacity?: V1beta1ResourceClaim_Spec_Devices_Requests_Capacity | Computed<V1beta1ResourceClaim_Spec_Devices_Requests_Capacity>;
  count?: number | Computed<number>;
  derivedAttributes?: V1beta1ResourceClaim_Spec_Devices_Requests_DerivedAttributes[] | Computed<V1beta1ResourceClaim_Spec_Devices_Requests_DerivedAttributes[]>;
  deviceClassName?: string | Computed<string>;
  name?: string | Computed<string>;
  selectors?: V1beta1ResourceClaim_Spec_Devices_Requests_FirstAvailable_Selectors[] | Computed<V1beta1ResourceClaim_Spec_Devices_Requests_FirstAvailable_Selectors[]>;
  tolerations?: V1beta1ResourceClaim_Spec_Devices_Requests_FirstAvailable_Tolerations[] | Computed<V1beta1ResourceClaim_Spec_Devices_Requests_FirstAvailable_Tolerations[]>;
}

export interface V1beta1ResourceClaim_Spec_Devices_Requests {
  adminAccess?: boolean | Computed<boolean>;
  allocationMode?: string | Computed<string>;
  capacity?: V1beta1ResourceClaim_Spec_Devices_Requests_Capacity | Computed<V1beta1ResourceClaim_Spec_Devices_Requests_Capacity>;
  count?: number | Computed<number>;
  derivedAttributes?: V1beta1ResourceClaim_Spec_Devices_Requests_DerivedAttributes[] | Computed<V1beta1ResourceClaim_Spec_Devices_Requests_DerivedAttributes[]>;
  deviceClassName?: string | Computed<string>;
  firstAvailable?: V1beta1ResourceClaim_Spec_Devices_Requests_FirstAvailable[] | Computed<V1beta1ResourceClaim_Spec_Devices_Requests_FirstAvailable[]>;
  name?: string | Computed<string>;
  selectors?: V1beta1ResourceClaim_Spec_Devices_Requests_FirstAvailable_Selectors[] | Computed<V1beta1ResourceClaim_Spec_Devices_Requests_FirstAvailable_Selectors[]>;
  tolerations?: V1beta1ResourceClaim_Spec_Devices_Requests_FirstAvailable_Tolerations[] | Computed<V1beta1ResourceClaim_Spec_Devices_Requests_FirstAvailable_Tolerations[]>;
}

export interface V1beta1ResourceClaim_Spec_Devices {
  /** This field holds configuration for multiple potential drivers which could satisfy requests in this claim. It is ignored while allocating the claim. */
  config?: V1beta1ResourceClaim_Spec_Devices_Config[] | Computed<V1beta1ResourceClaim_Spec_Devices_Config[]>;
  /** These constraints must be satisfied by the set of devices that get allocated for the claim. */
  constraints?: V1beta1ResourceClaim_Spec_Devices_Constraints[] | Computed<V1beta1ResourceClaim_Spec_Devices_Constraints[]>;
  /** Requests represent individual requests for distinct devices which must all be satisfied. If empty, nothing needs to be allocated. */
  requests?: V1beta1ResourceClaim_Spec_Devices_Requests[] | Computed<V1beta1ResourceClaim_Spec_Devices_Requests[]>;
}

export interface V1beta1ResourceClaim_Spec {
  /** DeviceClaim defines how to request devices with a ResourceClaim. */
  devices?: V1beta1ResourceClaim_Spec_Devices | Computed<V1beta1ResourceClaim_Spec_Devices>;
}

export interface V1beta1ResourceClaim_Status_Allocation_Devices_Config {
  opaque?: V1beta1ResourceClaim_Spec_Devices_Config_Opaque | Computed<V1beta1ResourceClaim_Spec_Devices_Config_Opaque>;
  requests?: string[] | Computed<string[]>;
  source?: string | Computed<string>;
}

export interface V1beta1ResourceClaim_Status_Allocation_Devices_Results {
  adminAccess?: boolean | Computed<boolean>;
  bindingConditions?: string[] | Computed<string[]>;
  bindingFailureConditions?: string[] | Computed<string[]>;
  consumedCapacity?: Record<string, string> | Computed<Record<string, string>>;
  device?: string | Computed<string>;
  driver?: string | Computed<string>;
  pool?: string | Computed<string>;
  request?: string | Computed<string>;
  shareId?: string | Computed<string>;
  skipNodeOperations?: string[] | Computed<string[]>;
  tolerations?: V1beta1ResourceClaim_Spec_Devices_Requests_FirstAvailable_Tolerations[] | Computed<V1beta1ResourceClaim_Spec_Devices_Requests_FirstAvailable_Tolerations[]>;
}

export interface V1beta1ResourceClaim_Status_Allocation_Devices {
  /** This field is a combination of all the claim and class configuration parameters. Drivers can distinguish between those based on a flag. This includes configuration parameters for drivers which have no allocated devices in the result because it is up to the drivers which configuration parameters they support. They can silently ignore unknown configuration parameters. */
  config?: V1beta1ResourceClaim_Status_Allocation_Devices_Config[] | Computed<V1beta1ResourceClaim_Status_Allocation_Devices_Config[]>;
  /** Results lists all allocated devices. */
  results?: V1beta1ResourceClaim_Status_Allocation_Devices_Results[] | Computed<V1beta1ResourceClaim_Status_Allocation_Devices_Results[]>;
}

export interface V1beta1ResourceClaim_Status_Allocation_NodeSelector_NodeSelectorTerms_MatchExpressions {
  key?: string | Computed<string>;
  operator?: string | Computed<string>;
  values?: string[] | Computed<string[]>;
}

export interface V1beta1ResourceClaim_Status_Allocation_NodeSelector_NodeSelectorTerms {
  matchExpressions?: V1beta1ResourceClaim_Status_Allocation_NodeSelector_NodeSelectorTerms_MatchExpressions[] | Computed<V1beta1ResourceClaim_Status_Allocation_NodeSelector_NodeSelectorTerms_MatchExpressions[]>;
  matchFields?: V1beta1ResourceClaim_Status_Allocation_NodeSelector_NodeSelectorTerms_MatchExpressions[] | Computed<V1beta1ResourceClaim_Status_Allocation_NodeSelector_NodeSelectorTerms_MatchExpressions[]>;
}

export interface V1beta1ResourceClaim_Status_Allocation_NodeSelector {
  /** Required. A list of node selector terms. The terms are ORed. */
  nodeSelectorTerms: V1beta1ResourceClaim_Status_Allocation_NodeSelector_NodeSelectorTerms[] | Computed<V1beta1ResourceClaim_Status_Allocation_NodeSelector_NodeSelectorTerms[]>;
}

export interface V1beta1ResourceClaim_Status_Allocation {
  /** Time is a wrapper around time.Time which supports correct marshaling to YAML and JSON. Wrappers are provided for many of the factory methods that the time package offers. */
  allocationTimestamp?: string | Computed<string>;
  /** DeviceAllocationResult is the result of allocating devices. */
  devices?: V1beta1ResourceClaim_Status_Allocation_Devices | Computed<V1beta1ResourceClaim_Status_Allocation_Devices>;
  /** A node selector represents the union of the results of one or more label queries over a set of nodes; that is, it represents the OR of the selectors represented by the node selector terms. */
  nodeSelector?: V1beta1ResourceClaim_Status_Allocation_NodeSelector | Computed<V1beta1ResourceClaim_Status_Allocation_NodeSelector>;
}

export interface V1beta1ResourceClaim_Status_Devices_Conditions {
  lastTransitionTime?: string | Computed<string>;
  message?: string | Computed<string>;
  observedGeneration?: number | Computed<number>;
  reason?: string | Computed<string>;
  status?: string | Computed<string>;
  type?: string | Computed<string>;
}

export interface V1beta1ResourceClaim_Status_Devices_NetworkData {
  hardwareAddress?: string | Computed<string>;
  interfaceName?: string | Computed<string>;
  ips?: string[] | Computed<string[]>;
}

export interface V1beta1ResourceClaim_Status_Devices {
  conditions?: V1beta1ResourceClaim_Status_Devices_Conditions[] | Computed<V1beta1ResourceClaim_Status_Devices_Conditions[]>;
  data?: unknown | Computed<unknown>;
  device?: string | Computed<string>;
  driver?: string | Computed<string>;
  networkData?: V1beta1ResourceClaim_Status_Devices_NetworkData | Computed<V1beta1ResourceClaim_Status_Devices_NetworkData>;
  pool?: string | Computed<string>;
  shareId?: string | Computed<string>;
}

export interface V1beta1ResourceClaim_Status_ReservedFor {
  apiGroup?: string | Computed<string>;
  name?: string | Computed<string>;
  resource?: string | Computed<string>;
  uid?: string | Computed<string>;
}

export interface V1beta1ResourceClaim_Status {
  /** AllocationResult contains attributes of an allocated resource. */
  allocation?: V1beta1ResourceClaim_Status_Allocation | Computed<V1beta1ResourceClaim_Status_Allocation>;
  /** Devices contains the status of each device allocated for this claim, as reported by the driver. This can include driver-specific information. Entries are owned by their respective drivers. */
  devices?: V1beta1ResourceClaim_Status_Devices[] | Computed<V1beta1ResourceClaim_Status_Devices[]>;
  /** ReservedFor indicates which entities are currently allowed to use the claim. A Pod which references a ResourceClaim which is not reserved for that Pod will not be started. A claim that is in use or might be in use because it has been reserved must not get deallocated. In a cluster with multiple scheduler instances, two pods might get scheduled concurrently by different schedulers. When they reference the same ResourceClaim which already has reached its maximum number of consumers, only one pod can be scheduled. Both schedulers try to add their pod to the claim.status.reservedFor field, but only the update that reaches the API server first gets stored. The other one fails with an error and the scheduler which issued it knows that it must put the pod back into the queue, waiting for the ResourceClaim to become usable again. There can be at most 256 such reservations. This may get increased in the future, but not reduced. */
  reservedFor?: V1beta1ResourceClaim_Status_ReservedFor[] | Computed<V1beta1ResourceClaim_Status_ReservedFor[]>;
}

const V1beta1ResourceClaim_Metadata_ManagedFieldsFields: FieldMap = {
  apiVersion: "api_version",
  fieldsType: "fields_type",
  fieldsV1: "fields_v1",
  manager: "manager",
  operation: "operation",
  subresource: "subresource",
  time: "time",
};

const V1beta1ResourceClaim_Metadata_OwnerReferencesFields: FieldMap = {
  apiVersion: "api_version",
  blockOwnerDeletion: "block_owner_deletion",
  controller: "controller",
  kind: "kind",
  name: "name",
  uid: "uid",
};

const V1beta1ResourceClaim_MetadataFields: FieldMap = {
  annotations: "annotations",
  creationTimestamp: "creation_timestamp",
  deletionGracePeriodSeconds: "deletion_grace_period_seconds",
  deletionTimestamp: "deletion_timestamp",
  finalizers: "finalizers",
  generateName: "generate_name",
  generation: "generation",
  labels: "labels",
  managedFields: {
    wireName: "managed_fields",
    kind: "list",
    fields: V1beta1ResourceClaim_Metadata_ManagedFieldsFields,
  },
  name: "name",
  namespace: "namespace",
  ownerReferences: {
    wireName: "owner_references",
    kind: "list",
    fields: V1beta1ResourceClaim_Metadata_OwnerReferencesFields,
  },
  resourceVersion: "resource_version",
  selfLink: "self_link",
  uid: "uid",
};

const V1beta1ResourceClaim_Spec_Devices_Config_OpaqueFields: FieldMap = {
  driver: "driver",
  parameters: "parameters",
};

const V1beta1ResourceClaim_Spec_Devices_ConfigFields: FieldMap = {
  opaque: {
    wireName: "opaque",
    kind: "object",
    fields: V1beta1ResourceClaim_Spec_Devices_Config_OpaqueFields,
  },
  requests: "requests",
};

const V1beta1ResourceClaim_Spec_Devices_ConstraintsFields: FieldMap = {
  distinctAttribute: "distinct_attribute",
  matchAttribute: "match_attribute",
  requests: "requests",
};

const V1beta1ResourceClaim_Spec_Devices_Requests_CapacityFields: FieldMap = {
  requests: "requests",
};

const V1beta1ResourceClaim_Spec_Devices_Requests_DerivedAttributesFields: FieldMap = {
  expression: "expression",
  name: "name",
};

const V1beta1ResourceClaim_Spec_Devices_Requests_FirstAvailable_Selectors_CelFields: FieldMap = {
  expression: "expression",
};

const V1beta1ResourceClaim_Spec_Devices_Requests_FirstAvailable_SelectorsFields: FieldMap = {
  cel: {
    wireName: "cel",
    kind: "object",
    fields: V1beta1ResourceClaim_Spec_Devices_Requests_FirstAvailable_Selectors_CelFields,
  },
};

const V1beta1ResourceClaim_Spec_Devices_Requests_FirstAvailable_TolerationsFields: FieldMap = {
  effect: "effect",
  key: "key",
  operator: "operator",
  tolerationSeconds: "toleration_seconds",
  value: "value",
};

const V1beta1ResourceClaim_Spec_Devices_Requests_FirstAvailableFields: FieldMap = {
  allocationMode: "allocation_mode",
  capacity: {
    wireName: "capacity",
    kind: "object",
    fields: V1beta1ResourceClaim_Spec_Devices_Requests_CapacityFields,
  },
  count: "count",
  derivedAttributes: {
    wireName: "derived_attributes",
    kind: "list",
    fields: V1beta1ResourceClaim_Spec_Devices_Requests_DerivedAttributesFields,
  },
  deviceClassName: "device_class_name",
  name: "name",
  selectors: {
    wireName: "selectors",
    kind: "list",
    fields: V1beta1ResourceClaim_Spec_Devices_Requests_FirstAvailable_SelectorsFields,
  },
  tolerations: {
    wireName: "tolerations",
    kind: "list",
    fields: V1beta1ResourceClaim_Spec_Devices_Requests_FirstAvailable_TolerationsFields,
  },
};

const V1beta1ResourceClaim_Spec_Devices_RequestsFields: FieldMap = {
  adminAccess: "admin_access",
  allocationMode: "allocation_mode",
  capacity: {
    wireName: "capacity",
    kind: "object",
    fields: V1beta1ResourceClaim_Spec_Devices_Requests_CapacityFields,
  },
  count: "count",
  derivedAttributes: {
    wireName: "derived_attributes",
    kind: "list",
    fields: V1beta1ResourceClaim_Spec_Devices_Requests_DerivedAttributesFields,
  },
  deviceClassName: "device_class_name",
  firstAvailable: {
    wireName: "first_available",
    kind: "list",
    fields: V1beta1ResourceClaim_Spec_Devices_Requests_FirstAvailableFields,
  },
  name: "name",
  selectors: {
    wireName: "selectors",
    kind: "list",
    fields: V1beta1ResourceClaim_Spec_Devices_Requests_FirstAvailable_SelectorsFields,
  },
  tolerations: {
    wireName: "tolerations",
    kind: "list",
    fields: V1beta1ResourceClaim_Spec_Devices_Requests_FirstAvailable_TolerationsFields,
  },
};

const V1beta1ResourceClaim_Spec_DevicesFields: FieldMap = {
  config: {
    wireName: "config",
    kind: "list",
    fields: V1beta1ResourceClaim_Spec_Devices_ConfigFields,
  },
  constraints: {
    wireName: "constraints",
    kind: "list",
    fields: V1beta1ResourceClaim_Spec_Devices_ConstraintsFields,
  },
  requests: {
    wireName: "requests",
    kind: "list",
    fields: V1beta1ResourceClaim_Spec_Devices_RequestsFields,
  },
};

const V1beta1ResourceClaim_SpecFields: FieldMap = {
  devices: {
    wireName: "devices",
    kind: "object",
    fields: V1beta1ResourceClaim_Spec_DevicesFields,
  },
};

const V1beta1ResourceClaim_Status_Allocation_Devices_ConfigFields: FieldMap = {
  opaque: {
    wireName: "opaque",
    kind: "object",
    fields: V1beta1ResourceClaim_Spec_Devices_Config_OpaqueFields,
  },
  requests: "requests",
  source: "source",
};

const V1beta1ResourceClaim_Status_Allocation_Devices_ResultsFields: FieldMap = {
  adminAccess: "admin_access",
  bindingConditions: "binding_conditions",
  bindingFailureConditions: "binding_failure_conditions",
  consumedCapacity: "consumed_capacity",
  device: "device",
  driver: "driver",
  pool: "pool",
  request: "request",
  shareId: "share_id",
  skipNodeOperations: "skip_node_operations",
  tolerations: {
    wireName: "tolerations",
    kind: "list",
    fields: V1beta1ResourceClaim_Spec_Devices_Requests_FirstAvailable_TolerationsFields,
  },
};

const V1beta1ResourceClaim_Status_Allocation_DevicesFields: FieldMap = {
  config: {
    wireName: "config",
    kind: "list",
    fields: V1beta1ResourceClaim_Status_Allocation_Devices_ConfigFields,
  },
  results: {
    wireName: "results",
    kind: "list",
    fields: V1beta1ResourceClaim_Status_Allocation_Devices_ResultsFields,
  },
};

const V1beta1ResourceClaim_Status_Allocation_NodeSelector_NodeSelectorTerms_MatchExpressionsFields: FieldMap = {
  key: "key",
  operator: "operator",
  values: "values",
};

const V1beta1ResourceClaim_Status_Allocation_NodeSelector_NodeSelectorTermsFields: FieldMap = {
  matchExpressions: {
    wireName: "match_expressions",
    kind: "list",
    fields: V1beta1ResourceClaim_Status_Allocation_NodeSelector_NodeSelectorTerms_MatchExpressionsFields,
  },
  matchFields: {
    wireName: "match_fields",
    kind: "list",
    fields: V1beta1ResourceClaim_Status_Allocation_NodeSelector_NodeSelectorTerms_MatchExpressionsFields,
  },
};

const V1beta1ResourceClaim_Status_Allocation_NodeSelectorFields: FieldMap = {
  nodeSelectorTerms: {
    wireName: "node_selector_terms",
    kind: "list",
    fields: V1beta1ResourceClaim_Status_Allocation_NodeSelector_NodeSelectorTermsFields,
  },
};

const V1beta1ResourceClaim_Status_AllocationFields: FieldMap = {
  allocationTimestamp: "allocation_timestamp",
  devices: {
    wireName: "devices",
    kind: "object",
    fields: V1beta1ResourceClaim_Status_Allocation_DevicesFields,
  },
  nodeSelector: {
    wireName: "node_selector",
    kind: "object",
    fields: V1beta1ResourceClaim_Status_Allocation_NodeSelectorFields,
  },
};

const V1beta1ResourceClaim_Status_Devices_ConditionsFields: FieldMap = {
  lastTransitionTime: "last_transition_time",
  message: "message",
  observedGeneration: "observed_generation",
  reason: "reason",
  status: "status",
  type: "type",
};

const V1beta1ResourceClaim_Status_Devices_NetworkDataFields: FieldMap = {
  hardwareAddress: "hardware_address",
  interfaceName: "interface_name",
  ips: "ips",
};

const V1beta1ResourceClaim_Status_DevicesFields: FieldMap = {
  conditions: {
    wireName: "conditions",
    kind: "list",
    fields: V1beta1ResourceClaim_Status_Devices_ConditionsFields,
  },
  data: "data",
  device: "device",
  driver: "driver",
  networkData: {
    wireName: "network_data",
    kind: "object",
    fields: V1beta1ResourceClaim_Status_Devices_NetworkDataFields,
  },
  pool: "pool",
  shareId: "share_id",
};

const V1beta1ResourceClaim_Status_ReservedForFields: FieldMap = {
  apiGroup: "api_group",
  name: "name",
  resource: "resource",
  uid: "uid",
};

const V1beta1ResourceClaim_StatusFields: FieldMap = {
  allocation: {
    wireName: "allocation",
    kind: "object",
    fields: V1beta1ResourceClaim_Status_AllocationFields,
  },
  devices: {
    wireName: "devices",
    kind: "list",
    fields: V1beta1ResourceClaim_Status_DevicesFields,
  },
  reservedFor: {
    wireName: "reserved_for",
    kind: "list",
    fields: V1beta1ResourceClaim_Status_ReservedForFields,
  },
};

export interface V1beta1ResourceClaimConfig {
  /** APIVersion defines the versioned schema of this representation of an object. Servers should convert recognized schemas to the latest internal value, and may reject unrecognized values. More info: https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#resources */
  apiVersion?: string | Computed<string>;
  /** Kind is a string value representing the REST resource this object represents. Servers may infer this from the endpoint the client submits requests to. Cannot be updated. In CamelCase. More info: https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#types-kinds */
  kind?: string | Computed<string>;
  /** ObjectMeta is metadata that all persisted resources must have, which includes all objects users must create. */
  metadata?: V1beta1ResourceClaim_Metadata | Computed<V1beta1ResourceClaim_Metadata>;
  /** ResourceClaimSpec defines what is being requested in a ResourceClaim and how to configure it. */
  spec?: V1beta1ResourceClaim_Spec | Computed<V1beta1ResourceClaim_Spec>;
  /** ResourceClaimStatus tracks whether the resource has been allocated and what the result of that was. */
  status?: V1beta1ResourceClaim_Status | Computed<V1beta1ResourceClaim_Status>;
  /** path parameter, not part of the API's own resource representation */
  namespace: string | Computed<string>;
  /** path parameter, not part of the API's own resource representation */
  name: string | Computed<string>;
}

export interface V1beta1ResourceClaimAttrs {
  /** APIVersion defines the versioned schema of this representation of an object. Servers should convert recognized schemas to the latest internal value, and may reject unrecognized values. More info: https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#resources */
  apiVersion: string;
  /** Kind is a string value representing the REST resource this object represents. Servers may infer this from the endpoint the client submits requests to. Cannot be updated. In CamelCase. More info: https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#types-kinds */
  kind: string;
  /** ObjectMeta is metadata that all persisted resources must have, which includes all objects users must create. */
  metadata: V1beta1ResourceClaim_Metadata;
  /** ResourceClaimSpec defines what is being requested in a ResourceClaim and how to configure it. */
  spec: V1beta1ResourceClaim_Spec;
  /** ResourceClaimStatus tracks whether the resource has been allocated and what the result of that was. */
  status: V1beta1ResourceClaim_Status;
  /** path parameter, not part of the API's own resource representation */
  namespace: string;
  /** path parameter, not part of the API's own resource representation */
  name: string;
}

export const V1beta1ResourceClaim: ResourceBinding<V1beta1ResourceClaimConfig, V1beta1ResourceClaimAttrs> = {
  wireType: "kubernetes_resource_v1beta1_resource_claim",
  fields: {
    apiVersion: "api_version",
    kind: "kind",
    metadata: {
      wireName: "metadata",
      kind: "object",
      fields: V1beta1ResourceClaim_MetadataFields,
    },
    spec: {
      wireName: "spec",
      kind: "object",
      fields: V1beta1ResourceClaim_SpecFields,
    },
    status: {
      wireName: "status",
      kind: "object",
      fields: V1beta1ResourceClaim_StatusFields,
    },
    namespace: "namespace",
    name: "name",
  },
};
