// Code generated by `ubx sdk gen` from kubernetes_service_v1. DO NOT EDIT.
package service

import ubx "github.com/ubiquex/ubx-sdk-go/runtime"

type ServiceV1_Metadata struct {
	Annotations any
	GenerateName any
	Generation any
	Labels any
	Name any
	Namespace any
	ResourceVersion any
	Uid any
}

type ServiceV1_Spec_Port struct {
	AppProtocol any
	Name any
	NodePort any
	Port any
	Protocol any
	TargetPort any
}

type ServiceV1_Spec_SessionAffinityConfig_ClientIp struct {
	TimeoutSeconds any
}

type ServiceV1_Spec_SessionAffinityConfig struct {
	ClientIp any
}

type ServiceV1_Spec struct {
	AllocateLoadBalancerNodePorts any
	ClusterIp any
	ClusterIps any
	ExternalIps any
	ExternalName any
	ExternalTrafficPolicy any
	HealthCheckNodePort any
	InternalTrafficPolicy any
	IpFamilies any
	IpFamilyPolicy any
	LoadBalancerClass any
	LoadBalancerIp any
	LoadBalancerSourceRanges any
	PublishNotReadyAddresses any
	Selector any
	SessionAffinity any
	Type any
	Port any
	SessionAffinityConfig any
}

type ServiceV1_Timeouts struct {
	Create any
}

var ServiceV1_MetadataFields = ubx.FieldMap{
		"Annotations": ubx.FieldSpec{WireName: "annotations"},
		"GenerateName": ubx.FieldSpec{WireName: "generate_name"},
		"Generation": ubx.FieldSpec{WireName: "generation"},
		"Labels": ubx.FieldSpec{WireName: "labels"},
		"Name": ubx.FieldSpec{WireName: "name"},
		"Namespace": ubx.FieldSpec{WireName: "namespace"},
		"ResourceVersion": ubx.FieldSpec{WireName: "resource_version"},
		"Uid": ubx.FieldSpec{WireName: "uid"},
	}

var ServiceV1_Spec_PortFields = ubx.FieldMap{
		"AppProtocol": ubx.FieldSpec{WireName: "app_protocol"},
		"Name": ubx.FieldSpec{WireName: "name"},
		"NodePort": ubx.FieldSpec{WireName: "node_port"},
		"Port": ubx.FieldSpec{WireName: "port"},
		"Protocol": ubx.FieldSpec{WireName: "protocol"},
		"TargetPort": ubx.FieldSpec{WireName: "target_port"},
	}

var ServiceV1_Spec_SessionAffinityConfig_ClientIpFields = ubx.FieldMap{
		"TimeoutSeconds": ubx.FieldSpec{WireName: "timeout_seconds"},
	}

var ServiceV1_Spec_SessionAffinityConfigFields = ubx.FieldMap{
		"ClientIp": ubx.FieldSpec{
			WireName: "client_ip",
			Kind: "list",
			Fields: ServiceV1_Spec_SessionAffinityConfig_ClientIpFields,
		},
	}

var ServiceV1_SpecFields = ubx.FieldMap{
		"AllocateLoadBalancerNodePorts": ubx.FieldSpec{WireName: "allocate_load_balancer_node_ports"},
		"ClusterIp": ubx.FieldSpec{WireName: "cluster_ip"},
		"ClusterIps": ubx.FieldSpec{WireName: "cluster_ips"},
		"ExternalIps": ubx.FieldSpec{WireName: "external_ips"},
		"ExternalName": ubx.FieldSpec{WireName: "external_name"},
		"ExternalTrafficPolicy": ubx.FieldSpec{WireName: "external_traffic_policy"},
		"HealthCheckNodePort": ubx.FieldSpec{WireName: "health_check_node_port"},
		"InternalTrafficPolicy": ubx.FieldSpec{WireName: "internal_traffic_policy"},
		"IpFamilies": ubx.FieldSpec{WireName: "ip_families"},
		"IpFamilyPolicy": ubx.FieldSpec{WireName: "ip_family_policy"},
		"LoadBalancerClass": ubx.FieldSpec{WireName: "load_balancer_class"},
		"LoadBalancerIp": ubx.FieldSpec{WireName: "load_balancer_ip"},
		"LoadBalancerSourceRanges": ubx.FieldSpec{WireName: "load_balancer_source_ranges"},
		"PublishNotReadyAddresses": ubx.FieldSpec{WireName: "publish_not_ready_addresses"},
		"Selector": ubx.FieldSpec{WireName: "selector"},
		"SessionAffinity": ubx.FieldSpec{WireName: "session_affinity"},
		"Type": ubx.FieldSpec{WireName: "type"},
		"Port": ubx.FieldSpec{
			WireName: "port",
			Kind: "list",
			Fields: ServiceV1_Spec_PortFields,
		},
		"SessionAffinityConfig": ubx.FieldSpec{
			WireName: "session_affinity_config",
			Kind: "list",
			Fields: ServiceV1_Spec_SessionAffinityConfigFields,
		},
	}

var ServiceV1_TimeoutsFields = ubx.FieldMap{
		"Create": ubx.FieldSpec{WireName: "create"},
	}

type ServiceV1Config struct {
	Id any
	WaitForLoadBalancer any
	Metadata any
	Spec any
	Timeouts any
}

var ServiceV1 = ubx.ResourceBinding{
	WireType: "kubernetes_service_v1",
	Fields: ubx.FieldMap{
		"Id": ubx.FieldSpec{WireName: "id"},
		"WaitForLoadBalancer": ubx.FieldSpec{WireName: "wait_for_load_balancer"},
		"Metadata": ubx.FieldSpec{
			WireName: "metadata",
			Kind: "list",
			Fields: ServiceV1_MetadataFields,
		},
		"Spec": ubx.FieldSpec{
			WireName: "spec",
			Kind: "list",
			Fields: ServiceV1_SpecFields,
		},
		"Timeouts": ubx.FieldSpec{
			WireName: "timeouts",
			Kind: "object",
			Fields: ServiceV1_TimeoutsFields,
		},
	},
}
